...
//Number of processes 
	static int total_process = 5;

//Number of resources 
	static int total_resource = 3;

//Function to find the need of each process 
	static void calculateNeed(int need[][], int maxm[][], int allot[][]) {

		// Calculating Need of each total_process
...
//Function to find the system is in safe state or not 
	static boolean isSafe(int processes[], int avail[], int maxm[][], int allot[][]) {
		int[][] need = new int[total_process][total_resource];

		// Function to calculate need matrix
		calculateNeed(need, maxm, allot);

		// Mark all processes as infinish
		boolean[] finish = new boolean[total_process];

		// To store safe sequence
		int[] safeSeq = new int[total_process];

		// Make a copy of available resources
		int[] work = new int[total_resource];
			...
			for (int p = 0; p < total_process; p++) {
				...
				if (finish[p] == false) {
					...
					if (j == total_resource) {
						...
						// current total_process to the available/work
						// resources i.e.free the resources
						for (int k = 0; k < total_resource; k++)
							work[k] += allot[p][k];
